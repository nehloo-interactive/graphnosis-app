...
export async function dispatch(deps: IpcDeps, method: string, params: unknown): Promise<unknown> {
  ...
  switch (method) {
    ...
    case 'activity.list': {
      ...
      const { events, actors, hasMore, nextCursor } = await deps.host.listOplogEventsForActivity({
    ...
    case 'llm:status': {
      ...
      const { resolveLlmCapabilities, resolveLlmTemperaturePreset, LLM_TEMPERATURE_PRESETS } = await import('@graphnosis-app/core').then(m => m.settings);
      const capabilities = resolveLlmCapabilities(settings);
      const temperaturePreset = resolveLlmTemperaturePreset(settings);
      ...
      return {
        ...
        capabilities,
        temperaturePreset,
        temperaturePresets: LLM_TEMPERATURE_PRESETS,
        catalog: LLM_CATALOG,
        backend,
      };
    }
    ...
    case 'ghampus:send': {
      ...
      return runGhampusSend(deps, params, {
        ...
        setPendingEngram: (v) => { ghampusPendingEngram = v; },
      });
    }
