...
/**
 * Strip the `(trained YYYY-MM-DD)` suffix a skill title carries.
 *
 * Call targets are recorded by title, and the fixtures are regenerated by
 * training the skills fresh, so an un-stripped date would bake "today" into the
 * fixture and drift on the next calendar day. This stayed invisible while call
 * resolution was broken — targetTitle was always absent — and surfaced the
 * moment calls started resolving again.
 */
function stripTrainedSuffix(title: string): string {
  return title.replace(/\s*\(trained \d{4}-\d{2}-\d{2}\)\s*$/, '').trim();
}
...
export function normalizeWalkPlanForGolden(plan: SkillExecutionPlan): GoldenWalkFixture {
  ...
  const steps = (procedural.length > 0 ? procedural : plan.steps).map((s) => ({
      ...
      ...(s.calls ? {
        calls: {
          targetTitle: stripTrainedSuffix(s.calls.targetTitle),
      ...
      ...(s.parallel?.length ? {
        parallel: s.parallel.map((p) => ({
          targetTitle: stripTrainedSuffix(p.targetTitle),
  ...
  if (plan.failureHandlers.length) {
    out.failureHandlers = plan.failureHandlers.map((h) => ({
      ...
      ...(h.targetTitle ? { targetTitle: stripTrainedSuffix(h.targetTitle) } : {}),
