...
## The consent gate — how it works and where to find it

Starting in Graphnosis **v0.10**, accessing `sensitive` engrams via any AI client requires **active in-app authorization by you** before any memory data leaves your device. Personal-tier engrams flow through without an extra prompt — installing Graphnosis and adding it to your AI client's MCP config is already two affirmative, informed actions for personal data. Users who want every personal access gated too can enable **Extra precaution mode** in Settings → AI (off by default). This page summarises the user-facing side; the complete technical design (in-app prompt, per-client policies, headless fallback, rate limit, session replay blocker, opt-in caps, audit trail) is in [AI Access Controls](/guides/ai-access-controls).

**Primary flow — one-click in-app prompt:** When a gated recall fires, the Graphnosis app pops a modal listing the AI client, the tier(s) requested, the AI provider's privacy policy link, and four buttons: **Deny**, **Allow once**, **Allow for 1 hour**, **Allow for today**. One click resolves the gate. The recall proceeds (or errors cleanly with "denied"). The grant is recorded in your cortex's audit log.

**Per-client default policy:** The first time a never-seen AI client triggers the gate, the app offers a one-time policy chooser per tier — always-allow, ask-with-grant-1h, ask-with-grant-1d, ask-every-time, never-allow. Saved policies are editable in Settings → AI → Client policies.

**Headless fallback — typed phrase:** When the in-app prompt cannot reach a window (sidecar running over SSH, in a Docker container, in CI), the gate falls back to the original mechanism: a **time-limited phrase** shown only in the Graphnosis app (Settings → AI → Consent Phrases) that you type into the AI conversation. The phrase is generated using `HMAC-SHA256(per-cortex-key, tier + ":" + window-number)` — algorithm public, key stored encrypted in your cortex and never transmitted. Personal-tier phrase rotates every 24 hours; sensitive-tier rotates every 1 hour. The phrase is never returned by any MCP tool and never in any log.

**Consent is cached:** Once you grant via the modal or phrase, Graphnosis remembers your authorization per (AI client, tier) for the duration you chose. You can revoke at any time in Settings → AI.

**What this mechanism cannot defend against:** An AI agent running on your machine with OS-level screen capture, clipboard access, or the macOS Accessibility API enabled could theoretically read the phrase from the Graphnosis app window — or, in extra-precaution mode, observe the modal. This is a different threat model — a compromised machine, not a misbehaving AI client — and no MCP-layer mechanism can fully prevent it. Graphnosis discloses this honestly: if you do not see the full consent prompt or notice in your Graphnosis app, do not authorize. Check Settings → AI to see current consent status.
