interface Env {
  ...
  /** AppImage filename on the release. Update this env var when a new
   *  version ships — no function redeploy needed. */
  LINUX_FILENAME?: string;
}

export const onRequestGet: PagesFunction<Env> = async ({ env }) => {
  const version = env.CURRENT_VERSION ?? 'latest';
  const filename = env.LINUX_FILENAME ?? 'Graphnosis_1.13.1_amd64.AppImage';
